"""
tools/ai_supervisor.py ΓÇö AI Active Supervisor for ALL Online Tasks.
...
logging.basicConfig(level=logging.INFO, format="[%(asctime)s] %(name)s %(levelname)s ΓÇö %(message)s")
...
class AISupervisor:
    """
    CENTRAL AI SUPERVISOR ΓÇö The brain that actively monitors ALL online tasks.
    ...
    def __init__(self):
        ...
        logger.info("≡ƒºá AI SUPERVISOR INITIALIZED ΓÇö Active monitoring ENGAGED")
    ...
    def register_task(self, task_type: str, target: str, marketplace: str = "", region: str = "India", **kwargs) -> str:
        ...
        logger.info(f"≡ƒºá AI SUPERVISOR: Task REGISTERED [{task_id}] ΓÇö {task_type} ΓåÆ {target}")
        return task_id
    ...
    def supervise(self, task_id: str):
        ...
        logger.info(f"≡ƒºá AI SUPERVISOR: Task STARTED [{task_id}] ΓÇö {task.task_type}")
        ...
        try:
            ...
            logger.info(f"≡ƒºá AI SUPERVISOR: Task COMPLETED [{task_id}]")
            ...
            logger.error(f"≡ƒºá AI SUPERVISOR: Task FAILED [{task_id}] ΓÇö {e}")
            raise
        finally:
            # Move to history
            with threading.Lock():
    ...
    # AI SCRAPER VALIDATOR ΓÇö Real-time validation of scraped data
    # ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
    ...
    def _ai_validate_scraped_products(self, task: OnlineTask):
        ...
        logger.info(f"≡ƒºá AI SUPERVISOR: Validating {len(products)} scraped products from [{task.task_id}]")
        ...
        for product in products:
            ...
            if validation.auto_soft_delete and product.get("product_id"):
                try:
                    ...
                    logger.info(f"≡ƒºá AI SUPERVISOR: Auto soft-deleted {product['product_id']} ΓÇö {validation.rejection_reason}")
...
{{
    ...
    def _ai_validate_discovery_output(self, task: OnlineTask):
        """AI validates discovery outputs (niches, sources, keywords) ΓÇö active implementation."""
        ...
        if not (sources or niches or keywords):
            return
        logger.info(f"≡ƒºá AI SUPERVISOR: Validating discovery [{task.task_id}] ΓÇö "
                    f"{len(sources)} sources, {len(niches)} niches, {len(keywords)} keywords")
        summary = (
            f"Sources: {len(sources)} ΓÇö sample: {[s.get('url','') for s in sources[:4]]}\n"
            f"Niches:  {len(niches)}  ΓÇö sample: {[n.get('category','') for n in niches[:4]]}\n"
            f"Keywords:{len(keywords)} ΓÇö sample: {[k.get('keyword',k) for k in keywords[:4]]}\n"
...
{{"sources_quality":"high|medium|low","niches_quality":"high|medium|low","keywords_quality":"high|medium|low","flagged_sources":[],"flagged_niches":[],"overall_quality_score":0-100,"ai_assessment":"...","recommended_adjustment":"..."}}"""
        ...
        try:
            ...
            task.ai_feedback.append(f"Discovery quality: {score}/100 ΓÇö {result.get('ai_assessment','OK')}")
            ...
            logger.info(f"≡ƒºá AI SUPERVISOR: Discovery quality {score}/100 [{task.task_id}]")
        except Exception as e:
            logger.warning(f"Discovery validation parse error: {e}")

    def _ai_validate_search_results(self, task: OnlineTask):
        """AI validates trend/search results ΓÇö active implementation."""
        data   = task.data_collected
        ...
        logger.info(f"≡ƒºá AI SUPERVISOR: Validating {len(trend_list)} trend signals [{task.task_id}]")
...
{{"valid_keywords":[],"invalid_keywords":[],"invalid_reasons":{{}},"overall_signal_quality":"high|medium|low","ai_assessment":"...","recommended_focus":"..."}}"""
        ...
        try:
            ...
            task.ai_feedback.append(
                f"Trend signals: {len(valid)} valid, {len(invalid)} filtered ΓÇö "
            ...
            logger.info(f"≡ƒºá AI SUPERVISOR: Trends {len(valid)} valid / {len(invalid)} filtered [{task.task_id}]")
        except Exception as e:
...
TASK: {task.task_type} ΓåÆ {task.target}
MARKETPLACE: {task.marketplace}
REGION: {task.region}
...
{{
    ...
    "strategy_adjustment": "overall strategy change recommendation"
        ...
        logger.info(f"≡ƒºá AI SUPERVISOR: Applied {len(improvements.get('improvements', []))} improvements to [{task.task_id}]")
    ...
    def start_monitoring(self):
        ...
        logger.info("≡ƒºá AI SUPERVISOR: Real-time monitoring STARTED")
    ...
    def stop_monitoring(self):
        ...
        logger.info("≡ƒºá AI SUPERVISOR: Real-time monitoring STOPPED")
                    ...
                    if task.status == TaskStatus.RUNNING:
                        ...
                        if task.started_at:
                            ...
                            if elapsed > 300:  # 5 minutes
                                ...
                                logger.warning(f"≡ƒºá AI SUPERVISOR: Task STUCK [{task.task_id}] ΓÇö {elapsed:.0f}s")
...
JSON response: {{"action": "retry|skip|reduce|change_strategy|alert", "reason": "...", "new_params": {{}}}}"""
        ...
        try:
            ...
            logger.info(f"≡ƒºá AI SUPERVISOR: Recovery suggestion for [{task.task_id}] ΓÇö {recovery.get('action')}")
        except:
            pass
...
JSON: {{"strategic_recommendations": [...], "priority_focus": "..."}}"""
        ...
        try:
            ...
            logger.info(f"≡ƒºá AI SUPERVISOR: Strategy review complete ΓÇö {len(strategy.get('strategic_recommendations', []))} recommendations")
        except:
            pass
    ...
    def validate_products_batch(self, products: List[Dict], marketplace: str, region: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"≡ƒºá AI SUPERVISOR batch validate [{marketplace}/{region}]: "
